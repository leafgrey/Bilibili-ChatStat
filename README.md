...
---
### ChatStat 1.2.0 更新日志  
1. 新增弹幕统计选项，支持忽略指定的弹幕，以及只统计指定的弹幕；
2. 支持自动开始及停止直播间爬虫；
3. 直播间爬虫面板“注册”按钮可以长按，长按时以零间隔爬取弹幕；
4. “清空日志”按钮长按可以强制刷新日志区；
5. 直播间爬虫面板新增“防手胡”选项；
6. 略微提升直播间爬取效率，完善异常处理机制，修复已知bug。  
### ChatStat 1.2.1 修订日志
1. 紧急修复未处理正则表达式匹配异常java.util.regex.PatternSyntaxException的bug，并在启动时告知用户；
2. 完善一些细节。
### ChatStat 1.2.2（最新版） 修订日志
1. 修复导入csv文件的bug；
2. 修改正则匹配逻辑，现在在处理<@ignore>和<@only>标签时，只要弹幕字符串的一部分匹配正则表达式就算作匹配。这意味着你不需要在正则表达式首尾加上任意字符的贪婪匹配；
3. “一个人发的多条相同弹幕只记一次”的默认状态改为关闭;
4. 完善一些其他细节。
---
...
### 爬取单视频
请在对应的文本框中键入视频的av号（无“av”前缀），然后点击`选定`。  
不知道怎么看av号？  
进入视频详情页，在浏览器地址栏中有如下所示的URL：  

    https://www.bilibili.com/video/av170001

av后面的数字便是av号（示例是辣♂个♂男♂人的视频）。  
### 爬取多视频
请准备好txt文件，格式要求每一行为一个av号（是否有“av”前缀无影响），不能有其他内容，如下：  
